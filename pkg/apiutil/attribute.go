...
func UnmarshalAttribute(attr *api.Attribute) (bgp.PathAttributeInterface, error) {
	...
	case *api.Attribute_NextHop:
		...
		return bgp.NewPathAttributeNextHop(nexthop), nil
	...
	case *api.Attribute_Aggregator:
		...
		return bgp.NewPathAttributeAggregator(a.Aggregator.Asn, address), nil
	...
	case *api.Attribute_OriginatorId:
		...
		return bgp.NewPathAttributeOriginatorId(id), nil
	case *api.Attribute_ClusterList:
		...
		return bgp.ParsePathAttributeClusterList(a.ClusterList.Ids), nil
	case *api.Attribute_MpReach:
		...
		attr := bgp.ParsePathAttributeMpReachNLRI(nexthop, nlris)
	...
	case *api.Attribute_As4Aggregator:
		...
		return bgp.NewPathAttributeAs4Aggregator(a.As4Aggregator.Asn, address), nil
	case *api.Attribute_PmsiTunnel:
		...
		case bgp.PMSI_TUNNEL_TYPE_INGRESS_REPL:
			...
			id = bgp.NewIngressReplTunnelID(ip)
		default:
		...
		for _, tlv := range a.TunnelEncap.Tlvs {
			...
			for _, tlv := range tlv.Tlvs {
				...
				case *api.TunnelEncapTLV_TLV_EgressEndpoint:
					subTlv = bgp.ParseTunnelEncapSubTLVEgressEndpoint(sv.EgressEndpoint.Address)
	...
	case *api.Attribute_Ip6ExtendedCommunities:
		...
		for _, an := range a.Ip6ExtendedCommunities.Communities {
			...
			case *api.IP6ExtendedCommunitiesAttribute_Community_Ipv6AddressSpecific:
				...
				community = bgp.ParseIPv6AddressSpecificExtended(bgp.ExtendedCommunityAttrSubType(v.SubType), v.Address, uint16(v.LocalAdmin), v.IsTransitive)
			case *api.IP6ExtendedCommunitiesAttribute_Community_RedirectIpv6AddressSpecific:
				v := an.GetRedirectIpv6AddressSpecific()
				community = bgp.ParseRedirectIPv6AddressSpecificExtended(v.Address, uint16(v.LocalAdmin))
			}
			if community == nil {
...
func MarshalSRv6TLVs(tlvs []bgp.PrefixSIDTLVInterface) ([]*api.PrefixSID_TLV, error) {
	var err error
	mtlvs := make([]*api.PrefixSID_TLV, len(tlvs))
	for i, tlv := range tlvs {
		...
		case *bgp.SRv6L3ServiceAttribute:
			...
			mtlvs[i].Tlv = &api.PrefixSID_TLV_L3Service{L3Service: o}
		case *bgp.SRv6ServiceTLV:
			...
			case bgp.TLVTypeSRv6L2Service:
				...
				mtlvs[i].Tlv = &api.PrefixSID_TLV_L2Service{L2Service: o}
			}
		default:
...
func UnmarshalRD(rd *api.RouteDistinguisher) (bgp.RouteDistinguisherInterface, error) {
	...
	case *api.RouteDistinguisher_IpAddress:
		rd := bgp.ParseRouteDistinguisherIPAddressAS(v.IpAddress.Admin, uint16(v.IpAddress.Assigned))
		if rd == nil {
	...
	for _, value := range values {
		...
		case *api.FlowSpecRule_IpPrefix:
			...
			case typ == bgp.FLOW_SPEC_TYPE_DST_PREFIX && isIPv4:
				rule = bgp.NewFlowSpecDestinationPrefix(bgp.ParseIPAddrPrefix(uint8(v.PrefixLen), v.Prefix))
			case typ == bgp.FLOW_SPEC_TYPE_SRC_PREFIX && isIPv4:
				rule = bgp.NewFlowSpecSourcePrefix(bgp.ParseIPAddrPrefix(uint8(v.PrefixLen), v.Prefix))
			case typ == bgp.FLOW_SPEC_TYPE_DST_PREFIX && !isIPv4:
				rule = bgp.NewFlowSpecDestinationPrefix6(bgp.ParseIPv6AddrPrefix(uint8(v.PrefixLen), v.Prefix), uint8(v.Offset))
			case typ == bgp.FLOW_SPEC_TYPE_SRC_PREFIX && !isIPv4:
				rule = bgp.NewFlowSpecSourcePrefix6(bgp.ParseIPv6AddrPrefix(uint8(v.PrefixLen), v.Prefix), uint8(v.Offset))
			}
...
func UnmarshalNLRI(rf bgp.RouteFamily, an *api.NLRI) (bgp.AddrPrefixInterface, error) {
	...
	case *api.NLRI_Prefix:
		...
		switch rf {
		case bgp.RF_IPv4_UC:
			nlri = bgp.ParseIPAddrPrefix(uint8(v.PrefixLen), v.Prefix)
		case bgp.RF_IPv6_UC:
			nlri = bgp.ParseIPv6AddrPrefix(uint8(v.PrefixLen), v.Prefix)
		}
	case *api.NLRI_LabeledPrefix:
		v := n.LabeledPrefix
		switch rf {
		case bgp.RF_IPv4_MPLS:
			nlri = bgp.ParseLabeledIPAddrPrefix(uint8(v.PrefixLen), v.Prefix, *bgp.NewMPLSLabelStack(v.Labels...))
		case bgp.RF_IPv6_MPLS:
			nlri = bgp.ParseLabeledIPv6AddrPrefix(uint8(v.PrefixLen), v.Prefix, *bgp.NewMPLSLabelStack(v.Labels...))
		}
	case *api.NLRI_Encapsulation:
		v := n.Encapsulation
		switch rf {
		case bgp.RF_IPv4_ENCAP:
			nlri = bgp.ParseEncapNLRI(v.Address)
		case bgp.RF_IPv6_ENCAP:
			nlri = bgp.ParseEncapv6NLRI(v.Address)
		}
	case *api.NLRI_Vpls:
		v := n.Vpls
		if rf == bgp.RF_VPLS {
	...
	case *api.NLRI_EvpnMacadv:
		...
		if rf == bgp.RF_EVPN {
			...
			nlri = bgp.ParseEVPNMacIPAdvertisementRoute(rd, *esi, v.EthernetTag, v.MacAddress, v.IpAddress, v.Labels)
		}
	case *api.NLRI_EvpnMulticast:
		...
		if rf == bgp.RF_EVPN {
			...
			nlri = bgp.ParseEVPNMulticastEthernetTagRoute(rd, v.EthernetTag, v.IpAddress)
		}
	case *api.NLRI_EvpnEthernetSegment:
		...
		if rf == bgp.RF_EVPN {
			...
			nlri = bgp.ParseEVPNEthernetSegmentRoute(rd, *esi, v.IpAddress)
		}
	case *api.NLRI_EvpnIpPrefix:
		...
		if rf == bgp.RF_EVPN {
			...
			nlri = bgp.ParseEVPNIPPrefixRoute(rd, *esi, v.EthernetTag, uint8(v.IpPrefixLen), v.IpPrefix, v.GwAddress, v.Label)
		}
	...
	case *api.NLRI_LabeledVpnIpPrefix:
		...
		case bgp.RF_IPv4_VPN:
			nlri = bgp.ParseLabeledVPNIPAddrPrefix(uint8(v.PrefixLen), v.Prefix, *bgp.NewMPLSLabelStack(v.Labels...), rd)
		case bgp.RF_IPv6_VPN:
			nlri = bgp.ParseLabeledVPNIPv6AddrPrefix(uint8(v.PrefixLen), v.Prefix, *bgp.NewMPLSLabelStack(v.Labels...), rd)
		}
...
func UnmarshalRT(rt *api.RouteTarget) (bgp.ExtendedCommunityInterface, error) {
	...
	case *api.RouteTarget_Ipv4AddressSpecific:
		...
		rt := bgp.ParseIPv4AddressSpecificExtended(bgp.ExtendedCommunityAttrSubType(v.SubType), v.Address, uint16(v.LocalAdmin), v.IsTransitive)
		if rt == nil {
...
func unmarshalExComm(a *api.ExtendedCommunitiesAttribute) (*bgp.PathAttributeExtendedCommunities, error) {
	...
	for _, c := range a.Communities {
		...
		case *api.ExtendedCommunity_Ipv4AddressSpecific:
			...
			community = bgp.ParseIPv4AddressSpecificExtended(bgp.ExtendedCommunityAttrSubType(v.SubType), v.Address, uint16(v.LocalAdmin), v.IsTransitive)
		...
		case *api.ExtendedCommunity_EsImport:
			v := comm.EsImport
			community = bgp.ParseESImportRouteTarget(v.EsImport)
		...
		case *api.ExtendedCommunity_RouterMac:
			v := comm.RouterMac
			community = bgp.ParseRoutersMacExtended(v.Mac)
		...
		case *api.ExtendedCommunity_RedirectIpv4AddressSpecific:
			...
			community = bgp.ParseRedirectIPv4AddressSpecificExtended(v.Address, uint16(v.LocalAdmin))
...
func MarshalPathAttributes(attrList []bgp.PathAttributeInterface) ([]*api.Attribute, error) {
	anyList := make([]*api.Attribute, 0, len(attrList))
